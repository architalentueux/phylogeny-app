<script>
	...
	onMount(async () => {
		const response = await fetch("/src/static/BrassiToL_metadata.json");
		metadata = await response.json();
		console.log("METADATA", metadata);

		const landcodeResponse = await fetch("/src/static/BrassiToL_landcodes.json");
...
<style>
	...
	.geographyDropdown > div > input[type="text"] {
		...
		padding-left: 1em;
		background-image: url("/src/static/img/magnifyingglass-small.png"); /* Path to your icon */
